...
if not st.session_state.logged_in:
    ...
    **Demo Accounts**
    ...
    Management:
    `management@utility.com` / `management123`

    Plant Head:
    `planthead@utility.com` / `planthead123`

    Engineer:
    `engineer@utility.com` / `engineer123`
    """)
...
st.sidebar.divider()
st.sidebar.info("Data period: Aug 01–18, 2026")
...
if st.session_state.user_role == "Management":
    ...
    available_pages = management_pages

elif st.session_state.user_role == "Plant Head":

    available_pages = plant_head_pages

else:

    available_pages = engineer_pages
...
st.sidebar.info(
    "Data period: Aug 01–18, 2026"
)

st.sidebar.caption(
    "Real Excel project data is used as the initial dataset."
)
    ...
    if uploaded:
        for file in uploaded:
            ...
            try:
                ...
                b.metric("Missing Cells", missing)
                c.metric("Duplicate Rows", duplicates)

                if st.button(f"Validate {file.name}", key=f"validate_{file.name}"):
                    st.success("File parsed successfully. Review the preview before importing it into the production database.")
